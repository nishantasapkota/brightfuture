...
export default async function CourseDetailPage({ params }: CourseDetailPageProps) {
  ...
  const sidebarItems = allCourses.filter((c) => c.slug !== slug).map((c) => ({ title: c.name, href: `/courses/${c.slug}` }))
          ...
          <div className="space-y-12">
            {course.icon && (
              <div className="relative h-[360px] md:h-[420px] w-full rounded-2xl overflow-hidden shadow-xl shadow-slate-900/5 group">
                <Image src={course.icon} alt={course.name} fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
              </div>
            )}
            ...
            {course.highlights && course.highlights.length > 0 && (
              <div className="space-y-6">
                <h3 className="text-xl font-bold text-slate-900">Course Highlights</h3>
