...
import { ArrowRight, CheckCircle2, ArrowUpRight, Clock, Users, Award, Phone } from "lucide-react"
...
export default async function ServiceDetailPage({ params }: ServiceDetailPageProps) {
  ...
  const sidebarItems = allServices.filter((item) => item.slug !== slug).map((item) => ({ title: item.name, href: `/services/${item.slug}`, icon: item.icon }))
      ...
      <div className="container py-20 md:py-24">
        <div className="grid lg:grid-cols-[1fr_360px] gap-16">
          <div className="space-y-14">
            {service.icon && (
              <div className="relative h-[300px] md:h-[400px] w-full rounded-3xl overflow-hidden shadow-xl shadow-slate-900/5 group">
            ...
            {/* Overview */}
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-6">Service Overview</h2>
              <div className="prose prose-lg max-w-none prose-headings:text-slate-900 prose-p:text-slate-500 prose-li:text-slate-500 prose-strong:text-slate-900" dangerouslySetInnerHTML={{ __html: service.description }} />
            </div>
            ...
            <div className="relative overflow-hidden rounded-3xl bg-slate-950 p-8 md:p-12 text-white">
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-6 md:gap-10">
                <div className="flex-1">
                  <h3 className="text-2xl font-bold mb-2">Interested in this service?</h3>
                  <p className="text-white/50">Book a free consultation and let our experts guide you.</p>
                </div>
                <Link href="/contact">
