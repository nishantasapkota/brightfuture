...
3. **Set up environment variables**
   ...
   **For MongoDB Atlas (Cloud - Recommended):**
   \`\`\`env
   MONGODB_URI=mongodb+srv://username:password@cluster.mongodb.net/brightfuture
   MONGODB_DB=brightfuture
   \`\`\`
   
   **For Local MongoDB:**
   \`\`\`env
   MONGODB_URI=mongodb://localhost:27017/brightfuture
   MONGODB_DB=brightfuture
   \`\`\`
...
4. Update `MONGODB_URI` in `.env.local` to `mongodb://localhost:27017/brightfuture`
